...
// Corrected imports: remove './dashboard/' subpath references:
import WelcomeHeader from "./WelcomeHeader";
import StatsGrid, { type Stat } from "./StatsGrid";
import RecentCampaigns from "./RecentCampaigns";
import RecentAssets, { type RecentAsset } from "./RecentAssets";
import ActivityLog from "./ActivityLog";
import AssetImportAnalyticsChart from "./AssetImportAnalyticsChart";
import AssetTypeBreakdownChart from "./AssetTypeBreakdownChart";
...
const Dashboard = ({ orgId, onGoToCampaignBuilder }: DashboardProps) => {
  ...
  // New: Asset Type Breakdown
  const assetTypeCounts = React.useMemo(() => {
    const typeMap: Record<string, number> = {};
    assets?.forEach(asset => {
      const type = fileUploadService.getFileType(asset.filename);
  ...
  const imageAssetsCount = assetTypeCounts.find(x => x.type === "image")?.count || 0;
  ...
  const stats: Stat[] = [
    ...
    { name: "Image Assets", value: imageAssetsCount, icon: Image, color: "text-indigo-600" }, // NEW
  ...
  return (
    <div className="space-y-6">
      <AssetImportAnalyticsChart />
      <AssetTypeBreakdownChart data={assetTypeCounts} />
